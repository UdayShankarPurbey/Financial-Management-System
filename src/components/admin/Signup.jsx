...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
   ...
   // creating user via web and link with db
    const response = await fetch("http://localhost:5000/api/admin/auth/createuser", {
            method: "POST",
            headers: {
          ...
          console.log({name : credentials.name, username: credentials.username, password: credentials.password})
    ...
    <>
      ...
      <div className="max-w-md w-full p-6 bg-white shadow-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Link className='p-4 hover:text-green-500 hover:underline' to={'/adminlogin'}>Already Have an Account</Link>
        </form>
      </div>
    </div>
    </>
